...
export default function ApproveListPage() {
    const [approveList, setApporveList] = useState([])
    ...
    useEffect(() => {
        ...
        async function fetchApprovePainting() {
            const response = await adminApi.getApprovePaintings(token);
            setApporveList(response)
        }

        fetchApprovePainting()
    }, [])
